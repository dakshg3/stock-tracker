...
export function exportPortfolioCSV(portfolio) {
  const header = "symbol,qty,avgPrice";
  const rows = portfolio.map((h) => `${h.symbol},${h.qty},${h.avgPrice}`);
...
export function exportWatchlistCSV(watchlist) {
  const header = "symbol";
  const rows = watchlist.map((s) => s);
  const csv = [header, ...rows].join("\n");
...
export function parseWatchlistCSV(text) {
  ...
  // skip header if present
  const start = lines[0]?.toLowerCase() === "symbol" ? 1 : 0;
